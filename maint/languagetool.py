...
def word_in(w, text):
    left_anchor = r'(\W|^)' if w.startswith('--') else r'\b'
    right_anchor = r'(\W|$)' if w.endswith('--') else r'\b'
    return bool(re.search(left_anchor + w + right_anchor, text))
...
while retries:
    p = subprocess.Popen(
        ...
        # ltex-cli-plus --server-command-line=ltex-ls-plus --hide-commands,
        # but more robust
        ['ltex-ls-plus', '--input-documents', *stripped_files],
        env={**os.environ, 'JAVA_OPTS': '-Xms2g -Xmx8g'},
    ...
    try:
        ...
        retries -= 1
        print(f'... timeout, restarting, {retries=}...')
        sys.stdout.flush()
        p.kill()
        continue
    now = time.time()
    errs = errs.replace('launchServer\nWARNING', 'launchServer WARNING')
    errs = '\n'.join([ln for ln in errs.split('\n')
                      if 'WARNING: --input-documents' not in ln])
    if errs:
        retries -= 1
        print(errs.strip()[:80].replace("\n", " "))
        print(f'... {p.returncode} {now - start:.3f}s, '
              f'restarting, {retries=}...')
        sys.stdout.flush()
        continue
    print(f'ltex completed in {now - start:.3f} sec, '
